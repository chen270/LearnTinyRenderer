...
void Triangle::triangle_scan(Vec2i t0, Vec2i t1, Vec2i t2, TGAImage &image, TGAColor color)
{
    ...
    float k02_x = (t2.x - t0.x) / static_cast<float>(t2.y - t0.y);
    float k01_x = (t1.x - t0.x) / static_cast<float>(t1.y - t0.y);
    float k12_x = (t2.x - t1.x) / static_cast<float>(t2.y - t1.y);
    for (int col = t0.y; col <= t2.y; ++col)
    {
        if (col <= t1.y)
        {
            // 下半部 t0.y - t1.y
            int x_min = static_cast<int>(k02_x * (col - t0.y) + t0.x);
            int x_max = static_cast<int>(k01_x * (col - t0.y) + t0.x);
            if (x_min > x_max)
        ...
        else
        {
            // 上半部 t2.y - t1.y
            int x_min = static_cast<int>(k02_x * (col - t0.y) + t0.x);
            int x_max = static_cast<int>(k12_x * (col - t1.y) + t1.x);
            if (x_min > x_max)
                std::swap(x_min, x_max);
            for (int row = x_min; row <= x_max; ++row)
                image.set(row, col, color);
...
static Vec3f barycentric(Vec2i* pts, Vec2i P)
{
    ...
    // P = (1-u-v)A + uB + vC
    float xU = u.x / static_cast<float>(u.z);
    float xV = u.y / static_cast<float>(u.z);
    float xW = (u.z - u.x - u.y) / static_cast<float>(u.z);
...
void Triangle::triangle_barycenter(std::vector<std::vector<float>> &zbuffer, Vec2i* pts2i, Vec3f* pts3f, TGAImage& image, TGAColor color)
{
    Vec2i t0 = pts2i[0];
    Vec2i t1 = pts2i[1];
    Vec2i t2 = pts2i[2];
...
Triangle::Triangle(const TGAImage &model, std::vector<std::vector<float>> &zbuffer, Vec3f *pts, Vec2i *uvs, TGAImage &image, float intensity)
{
    ...
    int min_x = static_cast<int>(std::floor(minx));
    int max_x = static_cast<int>(std::ceil(maxx));
    int min_y = static_cast<int>(std::floor(miny));
    int max_y = static_cast<int>(std::ceil(maxy));
